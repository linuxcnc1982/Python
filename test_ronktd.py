...
from telegram import InlineKeyboardButton, KeyboardButton, ReplyKeyboardMarkup, WebAppInfo, ReplyKeyboardRemove, \
    InlineKeyboardMarkup, Update, InputTextMessageContent, InlineQueryResultArticle, InlineQueryResultsButton
...
def get_prombez(FN: str, start_date, end_date) -> list:
    ...
    cursor = connection.cursor()
    req = (
        f"SELECT Education.Name,ExpDate,Certificate,Protocol FROM Prombez,Education WHERE USER={user_id} AND Prombez.Education=Education.Key AND ExpDateUs BETWEEN '{start_date}' AND '{end_date}'")
    print(req)
    try:
        cursor.execute(req)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == 'ronktd':
        print('ronktd')
        lst = get_cert(FIOsearch)

    if query.data == 'naks':
        print('naks')
        lst = get_cert_naks(FIOsearch)

    if query.data == 'prombez':
        print('prombez')
        lst = get_prombez(FIOsearch, get_date(), '2100-01-01')
    ...
    lst = prepare(lst, False)
    print(lst)
    # CallbackQueries need to be answered, even if no notification to the user is needed
    # Some clients may have trouble otherwise. See https://core.telegram.org/bots/api#callbackquery
    # await query.answer()
    # await query.edit_message_text(text=f"Selected option: {query.data}")
    await query.edit_message_text(text=f"👷 {FIOsearch} {lst}")
...
async def custom_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(lst)
    await update.message.reply_text(f'РОНКТД {lst}\nПромбез {list2} {list22}\nНАКС {list3}')
